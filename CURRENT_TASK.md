...
## Checklist
- [ ] `quiz.service.js` gọi đúng toàn bộ API student quiz.
- [ ] Route làm quiz và route result hoạt động.
- [ ] Quiz detail không phụ thuộc vào `isCorrect`.
- [ ] Start attempt hoạt động và lưu `attemptId`.
- [ ] Submit payload đúng format backend.
- [ ] Result page hiển thị score/passed/correct/wrong.
- [ ] Attempt history hiển thị tối thiểu ở dashboard/profile nếu phù hợp.
- [ ] Lỗi API được hiển thị thân thiện.
- [ ] Không hardcode quiz id trong production UI.
- [ ] Build/test frontend pass hoặc blocker được ghi rõ.
...
## Prompt gợi ý cho Antigravity

```text
Bạn đang làm trong repo JapaneseLearningWeb. Hãy đọc CURRENT_TASK.md và các docs được liệt kê trong đó trước khi code.

Nhiệm vụ: triển khai Frontend Student Quiz Taking Integration.

Backend đã có các API:
- GET /api/v1/quizzes/{id}
- POST /api/v1/quizzes/{id}/start
- POST /api/v1/quizzes/{id}/submit
- GET /api/v1/quizzes/{id}/result/{attemptId}
...
Yêu cầu:
1. Tạo/cập nhật quiz service ở frontend để gọi các API trên.
2. Thêm route student cho trang làm quiz và trang kết quả.
3. Tạo QuizTakingPage:
   - Load quiz detail.
   - Không phụ thuộc vào isCorrect trong response.
   - Start attempt trước khi submit.
   - Render SINGLE_CHOICE và TRUE_FALSE tốt trước.
   - Submit payload đúng contract backend.
   - Chuyển sang result page sau submit.
4. Tạo QuizResultPage:
   - Hiển thị score, passing score, passed/failed, correct/wrong count.
   - Hiển thị từng câu, đáp án đã chọn, đáp án đúng và explanation nếu có.
5. Tích hợp lịch sử quiz attempt vào dashboard/profile ở mức tối thiểu nếu phù hợp.
6. Không hardcode quiz id trong UI chính.
7. Nếu lesson learning flow chưa có cách discover quiz theo lesson, ghi rõ blocker/TODO thay vì hack.
8. Chạy frontend build/test phù hợp và ghi rõ kết quả.

Giữ scope gọn, bám style hiện có của frontend, không redesign toàn bộ website trong task này.
```
