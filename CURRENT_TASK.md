...
* Chỉ cấu hình phân quyền cơ bản theo role.
* Không code frontend.
* Không code Course.
* Không code Lesson.
* Không code Payment.
* Không code Quiz.
* Không code Admin CRUD thật.
* Không làm permission phức tạp theo resource owner.
* Không tự ý đổi kiến trúc project.

Trước khi code, hãy phân tích:

1. Task này cần tạo hoặc sửa những file nào?
2. Role hiện tại đang được map sang authority như thế nào?
3. SecurityConfig nên cấu hình public/authenticated/admin endpoint ra sao?
4. Có cần bật `@EnableMethodSecurity` không?
5. Có cần `AccessDeniedHandler` để xử lý 403 không?
6. Sự khác nhau giữa 401 và 403 trong task này là gì?
7. Có rủi ro gì khi map role sai format không?
8. Sau khi code xong tôi cần test bằng Swagger/Postman như thế nào?
...
* Dùng ErrorCode/AppException nếu project đang dùng.
* Swagger/OpenAPI description dùng English.
* Đảm bảo register/login/refresh-token/logout vẫn public.
* Đảm bảo `/api/users/me` vẫn authenticated.
* Đảm bảo `/api/admin/**` chỉ ADMIN/SUPER_ADMIN.
* Sau khi hoàn thành, hướng dẫn tôi test từng case:

  1. Public endpoint không cần token.
  2. `/api/users/me` không token → 401.
  3. `/api/users/me` token hợp lệ → 200.
  4. Student vào admin endpoint → 403.
  5. Admin vào admin endpoint → 200.
